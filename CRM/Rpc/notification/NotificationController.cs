...
using CRM.Services.MNotificationStatus;
...
namespace CRM.Rpc.notification
{
    public class NotificationController : RpcController
    {
       ...
       ,IHttpContextAccessor httpContextAccessor, DataContext _DataContext
        ...
        public async Task<ActionResult<List<Notification_AppUserDTO>>> FilterListAppUser([FromBody] Notification_AppUserFilterDTO Notification_AppUserFilterDTO)
        {
            if (UnAuthorization) return Forbid();
            if (!ModelState.IsValid)
                throw new BindException(ModelState);
        ...
        public async Task<ActionResult<List<Notification_NotificationStatusDTO>>> FilterListNotificationStatus([FromBody] Notification_NotificationStatusFilterDTO Notification_NotificationStatusFilterDTO)
        {
            if (UnAuthorization) return Forbid();
            if (!ModelState.IsValid)
                throw new BindException(ModelState);
        ...
        public async Task<ActionResult<List<Notification_OrganizationDTO>>> FilterListOrganization([FromBody] Notification_OrganizationFilterDTO Notification_OrganizationFilterDTO)
        {
            if (UnAuthorization) return Forbid();
            if (!ModelState.IsValid)
                throw new BindException(ModelState);
