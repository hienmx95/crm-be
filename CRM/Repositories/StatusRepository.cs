...
namespace CRM.Repositories
{
    ...
    public class StatusRepository : IStatusRepository
    {
        ...
        {
            List<Status> Statuses = await DataContext.Status.AsNoTracking()
            .Where(x => Ids.Contains(x.Id)).Select(x => new Status()
            {
                Id = x.Id,
                Code = x.Code,
                Name = x.Name,
        ...
        {
            Status Status = await DataContext.Status.AsNoTracking()
            .Where(x => x.Id == Id)
            .Select(x => new Status()
            {
                Id = x.Id,
