    ...
    public class KpiGeneralRepository : IKpiGeneralRepository
    {
        ...
        private IQueryable<KpiGeneralDAO> OrFilter(IQueryable<KpiGeneralDAO> query, KpiGeneralFilter filter)
        {
            ...
            foreach (KpiGeneralFilter KpiGeneralFilter in filter.OrFilter)
            {
                ...
                if (KpiGeneralFilter.UserId != null)
                {
                    if (KpiGeneralFilter.UserId.Equal != null)
                    {
                        AppUserDAO AppUserDAO = DataContext.AppUser.Where(p => p.Id == KpiGeneralFilter.UserId.Equal).FirstOrDefault();
                        OrganizationDAO OrganizationDAO = DataContext.Organization
                            .Where(o => o.Id == AppUserDAO.OrganizationId).FirstOrDefault();
                        queryable = queryable.Where(q => q.Employee.Organization.Path.StartsWith(OrganizationDAO.Path));
                    }
                    if (KpiGeneralFilter.UserId.NotEqual != null)
                    {
                        AppUserDAO AppUserDAO = DataContext.AppUser.Where(p => p.Id == KpiGeneralFilter.UserId.NotEqual).FirstOrDefault();
                        OrganizationDAO OrganizationDAO = DataContext.Organization
                            .Where(o => o.Id != AppUserDAO.OrganizationId).FirstOrDefault();
                        queryable = queryable.Where(q => !q.Employee.Organization.Path.StartsWith(OrganizationDAO.Path));
                    }
