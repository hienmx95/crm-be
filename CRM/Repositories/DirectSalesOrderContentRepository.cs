...
namespace CRM.Repositories
{
    public interface IDirectSalesOrderContentRepository
    {
        ...
        Task<List<DirectSalesOrderContent>> List(List<long> Ids);
    ...
    public class DirectSalesOrderContentRepository : IDirectSalesOrderContentRepository
    {
        ...
        private IQueryable<DirectSalesOrderContentDAO> DynamicFilter(IQueryable<DirectSalesOrderContentDAO> query, DirectSalesOrderContentFilter filter)
        {
            ...
            if (filter.DirectSalesOrderId != null && filter.DirectSalesOrderId.HasValue)
        ...
        private IQueryable<DirectSalesOrderContentDAO> OrFilter(IQueryable<DirectSalesOrderContentDAO> query, DirectSalesOrderContentFilter filter)
        {
            ...
            foreach (DirectSalesOrderContentFilter DirectSalesOrderContentFilter in filter.OrFilter)
            {
                ...
                if (DirectSalesOrderContentFilter.Id != null && DirectSalesOrderContentFilter.Id.HasValue)
                    queryable = queryable.Where(q => q.Id, DirectSalesOrderContentFilter.Id);
                if (DirectSalesOrderContentFilter.DirectSalesOrderId != null && DirectSalesOrderContentFilter.DirectSalesOrderId.HasValue)
                    queryable = queryable.Where(q => q.DirectSalesOrderId, DirectSalesOrderContentFilter.DirectSalesOrderId);
                if (DirectSalesOrderContentFilter.ItemId != null && DirectSalesOrderContentFilter.ItemId.HasValue)
                    queryable = queryable.Where(q => q.ItemId, DirectSalesOrderContentFilter.ItemId);
                if (DirectSalesOrderContentFilter.UnitOfMeasureId != null && DirectSalesOrderContentFilter.UnitOfMeasureId.HasValue)
                    queryable = queryable.Where(q => q.UnitOfMeasureId, DirectSalesOrderContentFilter.UnitOfMeasureId);
                if (DirectSalesOrderContentFilter.Quantity != null && DirectSalesOrderContentFilter.Quantity.HasValue)
                    queryable = queryable.Where(q => q.Quantity, DirectSalesOrderContentFilter.Quantity);
                if (DirectSalesOrderContentFilter.PrimaryUnitOfMeasureId != null && DirectSalesOrderContentFilter.PrimaryUnitOfMeasureId.HasValue)
                    queryable = queryable.Where(q => q.PrimaryUnitOfMeasureId, DirectSalesOrderContentFilter.PrimaryUnitOfMeasureId);
                if (DirectSalesOrderContentFilter.RequestedQuantity != null && DirectSalesOrderContentFilter.RequestedQuantity.HasValue)
                    queryable = queryable.Where(q => q.RequestedQuantity, DirectSalesOrderContentFilter.RequestedQuantity);
                if (DirectSalesOrderContentFilter.PrimaryPrice != null && DirectSalesOrderContentFilter.PrimaryPrice.HasValue)
                    queryable = queryable.Where(q => q.PrimaryPrice, DirectSalesOrderContentFilter.PrimaryPrice);
                if (DirectSalesOrderContentFilter.SalePrice != null && DirectSalesOrderContentFilter.SalePrice.HasValue)
                    queryable = queryable.Where(q => q.SalePrice, DirectSalesOrderContentFilter.SalePrice);
                if (DirectSalesOrderContentFilter.EditedPriceStatusId != null && DirectSalesOrderContentFilter.EditedPriceStatusId.HasValue)
                    queryable = queryable.Where(q => q.EditedPriceStatusId, DirectSalesOrderContentFilter.EditedPriceStatusId);
                if (DirectSalesOrderContentFilter.DiscountPercentage != null && DirectSalesOrderContentFilter.DiscountPercentage.HasValue)
                    queryable = queryable.Where(q => q.DiscountPercentage.HasValue).Where(q => q.DiscountPercentage, DirectSalesOrderContentFilter.DiscountPercentage);
                if (DirectSalesOrderContentFilter.DiscountAmount != null && DirectSalesOrderContentFilter.DiscountAmount.HasValue)
                    queryable = queryable.Where(q => q.DiscountAmount.HasValue).Where(q => q.DiscountAmount, DirectSalesOrderContentFilter.DiscountAmount);
                if (DirectSalesOrderContentFilter.GeneralDiscountPercentage != null && DirectSalesOrderContentFilter.GeneralDiscountPercentage.HasValue)
                    queryable = queryable.Where(q => q.GeneralDiscountPercentage.HasValue).Where(q => q.GeneralDiscountPercentage, DirectSalesOrderContentFilter.GeneralDiscountPercentage);
                if (DirectSalesOrderContentFilter.GeneralDiscountAmount != null && DirectSalesOrderContentFilter.GeneralDiscountAmount.HasValue)
                    queryable = queryable.Where(q => q.GeneralDiscountAmount.HasValue).Where(q => q.GeneralDiscountAmount, DirectSalesOrderContentFilter.GeneralDiscountAmount);
                if (DirectSalesOrderContentFilter.TaxPercentage != null && DirectSalesOrderContentFilter.TaxPercentage.HasValue)
                    queryable = queryable.Where(q => q.TaxPercentage.HasValue).Where(q => q.TaxPercentage, DirectSalesOrderContentFilter.TaxPercentage);
                if (DirectSalesOrderContentFilter.TaxAmount != null && DirectSalesOrderContentFilter.TaxAmount.HasValue)
                    queryable = queryable.Where(q => q.TaxAmount.HasValue).Where(q => q.TaxAmount, DirectSalesOrderContentFilter.TaxAmount);
                if (DirectSalesOrderContentFilter.Amount != null && DirectSalesOrderContentFilter.Amount.HasValue)
                ...
                case OrderType.ASC:
                    switch (filter.OrderBy)
                    {
                        ...
                        case DirectSalesOrderContentOrder.SalePrice:
                            ...
                            break;
                        case DirectSalesOrderContentOrder.EditedPriceStatus:
        ...
        private async Task<List<DirectSalesOrderContent>> DynamicSelect(IQueryable<DirectSalesOrderContentDAO> query, DirectSalesOrderContentFilter filter)
        {
            List<DirectSalesOrderContent> DirectSalesOrderContents = await query.Select(q => new DirectSalesOrderContent()
            {
                ...
                PrimaryPrice = filter.Selects.Contains(DirectSalesOrderContentSelect.PrimaryPrice) ? q.PrimaryPrice : default(decimal),
                SalePrice = filter.Selects.Contains(DirectSalesOrderContentSelect.SalePrice) ? q.SalePrice : default(decimal),
                EditedPriceStatusId = filter.Selects.Contains(DirectSalesOrderContentSelect.EditedPriceStatus) ? q.EditedPriceStatusId : default(long),
                ...
                DiscountAmount = filter.Selects.Contains(DirectSalesOrderContentSelect.DiscountAmount) ? q.DiscountAmount : default(decimal?),
                ...
                GeneralDiscountAmount = filter.Selects.Contains(DirectSalesOrderContentSelect.GeneralDiscountAmount) ? q.GeneralDiscountAmount : default(decimal?),
                TaxPercentage = filter.Selects.Contains(DirectSalesOrderContentSelect.TaxPercentage) ? q.TaxPercentage : default(decimal?),
                TaxAmount = filter.Selects.Contains(DirectSalesOrderContentSelect.TaxAmount) ? q.TaxAmount : default(decimal?),
                Amount = filter.Selects.Contains(DirectSalesOrderContentSelect.Amount) ? q.Amount : default(decimal),
                ...
                DirectSalesOrder = filter.Selects.Contains(DirectSalesOrderContentSelect.DirectSalesOrder) && q.DirectSalesOrder != null ? new DirectSalesOrder
                {
                    ...
                    CreatorId = q.DirectSalesOrder.CreatorId,
                    ...
                    TotalAfterTax = q.DirectSalesOrder.TotalAfterTax,
                    PromotionCode = q.DirectSalesOrder.PromotionCode,
                    PromotionValue = q.DirectSalesOrder.PromotionValue,
                    Total = q.DirectSalesOrder.Total,
                    RowId = q.DirectSalesOrder.RowId,
                } : null,
                EditedPriceStatus = filter.Selects.Contains(DirectSalesOrderContentSelect.EditedPriceStatus) && q.EditedPriceStatus != null ? new EditedPriceStatus
                {
        ...
        public async Task<List<DirectSalesOrderContent>> List(List<long> Ids)
        {
            List<DirectSalesOrderContent> DirectSalesOrderContents = await DataContext.DirectSalesOrderContent.AsNoTracking()
            .Where(x => Ids.Contains(x.Id)).Select(x => new DirectSalesOrderContent()
            {
                Id = x.Id,
