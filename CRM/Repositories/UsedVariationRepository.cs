...
namespace CRM.Repositories
{
    public interface IUsedVariationRepository
    {
        ...
        Task<List<UsedVariation>> List(List<long> Ids);
    ...
    public class UsedVariationRepository : IUsedVariationRepository
    {
        ...
        private IQueryable<UsedVariationDAO> OrFilter(IQueryable<UsedVariationDAO> query, UsedVariationFilter filter)
        {
            ...
            foreach (UsedVariationFilter UsedVariationFilter in filter.OrFilter)
            {
                ...
                if (UsedVariationFilter.Id != null && UsedVariationFilter.Id.HasValue)
                    queryable = queryable.Where(q => q.Id, UsedVariationFilter.Id);
                if (UsedVariationFilter.Code != null && UsedVariationFilter.Code.HasValue)
                    queryable = queryable.Where(q => q.Code, UsedVariationFilter.Code);
                if (UsedVariationFilter.Name != null && UsedVariationFilter.Name.HasValue)
        ...
        public async Task<List<UsedVariation>> List(List<long> Ids)
        {
            List<UsedVariation> UsedVariations = await DataContext.UsedVariation.AsNoTracking()
            .Where(x => Ids.Contains(x.Id)).Select(x => new UsedVariation()
            {
                Id = x.Id,
                Code = x.Code,
                Name = x.Name,
